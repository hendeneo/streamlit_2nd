...
def check_for_duplicates(merged_gdf):
    ...
    has_duplicates = before_dedup > after_dedup
    return has_duplicates, deduped_gdf

def main():
    st.title("Merge and Check GeoJSON Files")

    st.write("Upload a ZIP folder containing multiple GeoJSON files. The app will merge them and check for duplicates.")
    ...
    if uploaded_zip is not None:
        with st.spinner("Processing..."):
            ...
            has_duplicates, deduped_gdf = check_for_duplicates(merged_gdf)

            if has_duplicates:
                st.warning("Duplicate fields across files")

            # Convert GeoDataFrame to DataFrame for display (excluding geometry column)
            df = deduped_gdf.drop(columns='geometry')
            ...
            deduped_gdf.to_file(geojson_output, driver='GeoJSON')
